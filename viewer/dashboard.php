...
if (!$conn) {
    ...
    // just show a blank dashboard rather than crashing
    $conn = null;
}

// default values in case the database is down
$enrolled_count      = 0;
$completed_count     = 0;
$in_progress_count   = 0;
...
if ($conn) {
    // how many unique tutorials has this student interacted with
    ...
    $enrolled_count = (int)($enrolledStmt->fetch()['count'] ?? 0);

    // completed count comes from user stats
    $completed_count   = $user_stats['completed_tutorials'] ?? 0;
    $in_progress_count = $enrolled_count - $completed_count;

    // total learning time from all completed tutorials
    ...
    // Bug 36 fix: smarter progress — 100% complete, 75% recent, 50% last month, 25% older
    ...
    // get the newest published tutorials to recommend
