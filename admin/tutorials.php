...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrfFromPost()) {
    ...
    if ($tutorial_id) {
        if ($action === 'delete') {
            // spec: "Remove content (with system message shown)"
...
<div class="admin-wrap"><main class="admin-main">
    ...
    <form method="GET" class="filter-bar">
        ...
        <select name="category">
            ...
            <option value="<?= $cat['category_id'] ?>" <?= $category_f===$cat['category_id']?'selected':'' ?>><?= e($cat['category_name']) ?></option>
            <?php endforeach; ?>
        </select>
                ...
                <td>
                    <div style="display:flex;gap:6px;">
                        ...
                        <form method="POST" style="display:inline;"
                            ...
                            <button class="btn-icon danger" title="Archive / Remove"><i class="fas fa-archive"></i></button>
                        </form>
                        <?php endif; ?>
                    </div>
                </td>
